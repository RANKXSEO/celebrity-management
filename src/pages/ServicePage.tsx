    ...
    <PageLayout>
      ...
      <section className="py-[clamp(52px,7vw,80px)] bg-background">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <RelatedLinks
            title="Other Reputation Management Services"
            siblings={servicePages
              .filter(s => s.slug !== page.slug)
              .slice(0, 3)
              .map(s => `/services/${s.slug}`)}
            hubUp={{ to: "/services", label: "Browse every celebrity ORM service" }}
          />

          {/* Reverse-silo: contextual cross-silo bridge to solutions */}
          <RelatedLinks
            title="Reputation Problems This Service Solves"
            siblings={page.relatedServices.slice(0, 2).map(rs => `/services/${rs.slug}`)}
            bridges={["/solutions/defamation-fake-news-removal", "/solutions/scandal-recovery"]}
