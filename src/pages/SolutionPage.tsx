    ...
    <PageLayout>
      ...
      <section className="py-[clamp(52px,7vw,80px)] bg-background">
        <div className="max-w-[800px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <RelatedLinks
            title="Other Reputation Solutions We Handle"
            siblings={solutionPages
              .filter(s => s.slug !== page.slug)
              .slice(0, 3)
              .map(s => `/solutions/${s.slug}`)}
            hubUp={{ to: "/solutions", label: "Browse every reputation solution" }}
          />

          {/* Reverse-silo: contextual services bridge (from relatedServices data) */}
          <RelatedLinks
            title="Services Powering This Solution"
            siblings={page.relatedServices.slice(0, 3).map(rs => `/services/${rs.slug}`)}
            hubUp={{ to: "/services", label: "See the full service catalogue" }}
          />

          <div className="bg-primary rounded-2xl p-8 text-center">
            <h3 className="font-display text-2xl text-primary-foreground font-bold mb-3">Don't Wait — Every Day Matters</h3>
            <p className="text-primary-foreground/75 mb-5">Get a free, confidential audit. We'll tell you exactly what's achievable and how long it takes.</p>
